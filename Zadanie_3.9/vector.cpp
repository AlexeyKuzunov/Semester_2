...
vector::vector(int length) : len(0), v(nullptr) {	// онструктор	
	assert(length >= 0);
	if (length > 0)
		v = new int[length];
	if (v) 
...
vector::vector(const vector& temp) : v(0), len(0) {// онструктор копировани€ Цконструктор, создающий объект (типа vector)
